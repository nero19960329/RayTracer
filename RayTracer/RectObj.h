...
public:
	RectObj() : Plane() {}
	RectObj(const std::shared_ptr<Texture> &_texture, int _num, const Vec3 &_center, const Vec3 &_x, const Vec3 &_y, real_t _length_x, real_t _length_y) :
		Plane(_texture, _num, _center, _x, _y), rectangle(_center, _x, _y, _length_x, _length_y) {}
	RectObj(const std::shared_ptr<Texture> &_texture, int _num, const RectGeo &rect) :
		Plane(_texture, _num, rect.center, rect.x, rect.y), rectangle(rect) {}
