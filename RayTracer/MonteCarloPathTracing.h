...
private:
	std::pair<Vec3, Vec3> getColor(DistRay &ray, int depth = 0) const;
	std::pair<Vec3, Vec3> getReflectedRadiance(DistRay &ray, const IntersectInfo &info, int depth = 0) const;
	Vec3 getDirectRadiance(DistRay &ray, const IntersectInfo &info) const;
	Vec3 getIndirectRadiance(DistRay &ray, const IntersectInfo &info, int depth = 0) const;
};