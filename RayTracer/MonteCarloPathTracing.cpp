...
Vec3 MonteCarloPathTracing::getDirectRadiance(DistRay &ray, const IntersectInfo &info) const {
	...
	return brdf->eval(inDir, outDir, info.normal, info.surface, DIFFUSE).mul(newInfo.surface->emission) * std::max(0.0, outDir.dot(info.normal)) * std::max(0.0, (-outDir).dot(newInfo.normal)) /
		((info.interPoint - newInfo.interPoint).sqr() * (2.0 / (130 * 105)));
}

Vec3 MonteCarloPathTracing::getIndirectRadiance(DistRay &ray, const IntersectInfo &info, int depth) const {
	real_t p = RUSSIAN_ROULETTE_POSSIBILITY;
	...
	if (pdf > epsilon) return brdf->eval(inDir, outDir, info.normal, info.surface, reflType).mul(radiance.first + radiance.second) * std::max(0.0, outDir.dot(info.normal)) / (p * pdf);
